...
data<- data[,-1] #deletel the column wich was used to merge
...
tidy_data<- dcast(data,features+activity~subject,mean)
tidy_data$subject<- as.numeric(tidy_data$subject)
tidy_data<-pivot_longer(tidy_data,3:32,"subject")
tidy_data<-data.table(tidy_data)


write.table(tidy_data,file="./tidydata")
